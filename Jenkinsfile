...
pipeline {
    ...
    environment {
        DOCKER_USERNAME = "ayambathie" // username docker
    ...
    stages {
        ...
        stage("Checkout") {
            steps {
                git branch: 'master', url: 'https://github.com/Aya2025-cmyk/jenkis.git'
            }
        }
        ...
        stage("Push image to Docker Hub") {
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'aya', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASSWORD')]){
                    sh """
